...
parser.add_argument('-d', '--downscale',
                    help="Downscale ratio", default=1.5, type=float)
parser.add_argument('-v', '--visualize',
                    help="Visualize the sliding window", action="store_true")
parser.add_argument('-w', '--winstride',
                    help="Pixels to move in one step, in any direction", default=8, type=int)
parser.add_argument('-n', '--nms_threshold',
                    help="Threshold Values between 0 to 1 for NMS thresholding. Default is 0.2", default=0.2, type=float)
...
while (h >= 128 and w >= 64):
    ...
    gray = cv2.resize(gray, (int(w*inverse), int(h*inverse)),
